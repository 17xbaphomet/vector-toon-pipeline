"""Stream: organic features + combinable FeatureProps."""
...
class ContinuousWalkStream:
    def __init__(self, scene, renderer, rig, config=None, route=None):
        ...
        self.cfg = config or StreamConfig(fps=float(scene.fps), width=scene.width, height=scene.height, scale=getattr(rig, "default_scale", 1.15), character_id=rig.id)
    ...
    def _compose_state(self, t):
        ...
        return FrameState(time=t, character_id=self.cfg.character_id, viseme=Viseme.X, jaw_open=0.0, bone_transforms=bones, root_position=(self._char_sx, self._char_sy), root_rotation_deg=0.0 if self.cfg.facing > 0 else 180.0, scale=self.cfg.scale * self.cfg.facing, camera=CameraState())
    ...
    def _make_sky_canvas(self, cel):
        ...
        for y in range(h):
            ...
            if t < 0.55:
                u = t / 0.55
                r, g, b = int(top[0]+(bot[0]-top[0])*u), int(top[1]+(bot[1]-top[1])*u), int(top[2]+(bot[2]-top[2])*u)
            else:
                u = (t - 0.55) / 0.45
                r, g, b = int(bot[0]*(1-0.15*u)), int(bot[1]*(1-0.15*u)), int(bot[2]*(1-0.15*u))
            draw.line([(0, y), (w, y)], fill=(r, g, b))
        return sky.convert("RGBA")

    def _draw_projected_stars(self, canvas, cel):
        if cel.sun_alt_deg > 0: return canvas
    ...
    def _draw_moon(self, canvas, cel):
        ...
        x, y = pos
        r = 22
        moon_lit, moon_dark = (230, 230, 210, 240), (230, 230, 210, 18)
        pa = cel.moon_phase * 2.0 * math.pi
        ...
        for py in range(r * 2 + 2):
            for px in range(r * 2 + 2):
                ...
                moon_img.putpixel((px, py), moon_lit if dx >= term else moon_dark)
        layer = Image.new("RGBA", canvas.size, (0, 0, 0, 0))
    ...
    def _blit_item(self, canvas, item, body_x):
        if not item.path.is_file(): return canvas
        left = self._world_to_screen_x(item.start, body_x, item.parallax)
        right = self._world_to_screen_x(item.end, body_x, item.parallax)
        if self.cfg.facing < 0: left, right = right, left
        if right < -60 or left > self.cfg.width + 60: return canvas
        art = self.renderer._svg_to_pil(item.path)
        if art.mode != "RGBA": art = art.convert("RGBA")
        if item.props is not None:
            art = apply_props_to_image(art, item.props)
    ...
    def _blit_tiled_layer(self, canvas, layer, state):
        ...
        if not path.is_file(): return canvas
        ...
        if layer.repeat_x:
            iw = img.size[0]
            if iw <= 0: return canvas
            x = (int(ox) % iw) - iw
            while x < w + iw:
                result.alpha_composite(img, (x, int(oy))); x += iw
        else:
            result.alpha_composite(img, (int(ox), int(oy)))
        return result

    def _collect_draw_items(self, body_x):
        return [_DrawItem(parallax=f.parallax, path=self.route.feature_object_path(f.kind), start=f.start, end=f.end, scale=f.scale, y_offset=f.y_offset, props=f.props) for f in self.route.active_features(body_x)]

    def _draw_ortsschild(self, img, text, screen_x, ground_y):
        if not text or screen_x < -100 or screen_x > img.width + 100: return img
        ...
        draw.rounded_rectangle([x0, y0, x0 + w, y0 + h], radius=4, fill=(245, 245, 245, 255), outline=(25, 25, 25, 255), width=3)
    ...
    def frames(self):
        ...
        frame_i = 0
        base_layers = sorted(self.route.base_layers(self._scroll_speed, self.cfg.facing), key=lambda L: L.parallax)
        while self._running:
            t = frame_i * dt
            if self.cfg.duration is not None and t >= self.cfg.duration: break
            ...
            if frame_i % 48 == 0: self.route.prune_behind(body_x, keep_behind=10000.0)
            ...
            for item in items:
                while bi < len(base_layers) and base_layers[bi].parallax <= item.parallax:
                    canvas = self._blit_tiled_layer(canvas, base_layers[bi], state); bi += 1
            ...
            yield self._apply_grade(frame, cel).convert("RGB")
            frame_i += 1
...
def run_mjpeg_server(stream, host="0.0.0.0", port=8765):
    ...
    print(f"MJPEG → http://{host}:{port}/ · {len(stream.route.features)} Features + Varianten")
    return server
...
def pipe_to_ffmpeg(stream, output="pipe:1", extra_args=None):
    ...
    try:
        for jpeg in stream.frames_jpeg(quality=80):
            proc.stdin.write(jpeg); proc.stdin.flush()
    except BrokenPipeError: pass
    finally:
        stream.stop(); proc.stdin.close(); return proc.wait()
