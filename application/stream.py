...
class ContinuousWalkStream:
    ...
    def _blit_item(self, canvas, item, body_x):
        ...
        # Bottom-align trimmed content so visual underside sits on ground plane
        # (get_sized_rgba already crop-trims transparent padding; returned h may be < panel_h)
        sw, sh = scaled.size
        y = int(self.cfg.height - sh + item.y_offset)
        x = int(left + (panel_w - sw) * 0.5)  # center within span
        if canvas.mode != "RGBA":
